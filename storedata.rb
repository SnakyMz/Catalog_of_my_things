...
def store_books(class_book)
  ...
  class_book.each do |book|
    books << { publisher: book.publisher, cover_state: book.cover_state, published_date: book.published_date, genre: book.genre.name }
  end
...
def store_musics(class_music)
  ...
  class_music.each do |music|
    musics << { published_date: music.published_date, on_spotify: music.on_spotify, genre: music.genre.name, title: music.label.title, color: music.label.color, first_name: music.author.first_name, last_name: music.author.last_name  }
  end
...
def store_games(class_game)
  ...
  class_game.each do |game|
    games << { multiplayer: game.multiplayer, last_played_at: game.last_played_at, published_date: game.published_date, genre: game.genre.name, title: game.label.title, color: game.label.color, first_name: game.author.first_name, last_name: game.author.last_name }
  end
